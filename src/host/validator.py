...
def validate_plan(plan: Dict[str, Any], tool_catalog: Dict[str, Any]) -> Tuple[str, str, Dict[str, Any]]:
    """
    tool_catalog format is the same JSON we provided to the model:
      { server: [ {name, inputSchema, ...}, ... ] }

    Returns (server, tool, args) if valid.
    ...
    if ptype == "final_answer":
        # In single-step mode, final_answer must set needs_more_info=true
